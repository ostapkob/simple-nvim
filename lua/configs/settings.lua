...
opt.softtabstop = 4 -- при использовании Backspace удалять по 4 пробела как один таб
...
vim.api.nvim_create_autocmd("FileType", {
	...
	pattern = { "xml", "html", "xhtml", "css", "scss", "javascript", "lua", "yaml", "yml", "jinja", "conf", "groovy" },
	callback = function()
...
-- Настройка отображения диагностических сообщений
vim.diagnostic.config({
	virtual_text = false, -- Показывать текст ошибок рядом с кодом
	signs = true, -- Показывать значки на полях
	underline = false, -- Подчеркивать проблемные места
	update_in_insert = false,
	severity_sort = true,
})

-- Показывать диагностическое сообщение при наведении курсора
