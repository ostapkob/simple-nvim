...
keymap.set(
	...
	":update<CR>:source %<CR>",
	default_opts,
	{ desc = "Сохранить и перезагрузить конфиг" }
)
...
keymap.set("v", "<C-r>", function()
	...
	vim.api.nvim_feedkeys(
		vim.api.nvim_replace_termcodes("<Esc>:%s/" .. escaped .. "//gc<Left><Left><Left>", true, false, true),
		"n",
		true
	)
...
vim.api.nvim_create_autocmd("LspAttach", {
	callback = function(args)
		keymap.set("n", "<leader>lf", vim.lsp.buf.format, { buffer = args.buf }, { desc = "[L]SP [F]ormat" })
	end,
})

-- Показать все ошибки в quickfix list
keymap.set("n", "<leader>ld", "<cmd>Trouble diagnostics toggle<CR>", { desc = "[L]SP [D]iagnostics" })
